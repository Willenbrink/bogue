...
and room = {
  ...
  (* What to do when the house changed size. (int * int) is the house size. *)
...
let create_win_house windows =
  create ~set_house:false ~name:"windows_house" (geometry ()) (Rooms windows)
...
let create = create ~set_house:true
...
let set_size ?(keep_resize = false) ?(check_window = true) ?(update_bg = false) ?w ?h l  =
  lock l;
  let () = match w,h with
    | Some w, Some h ->
...
let unload_widget_textures room =
  ...
  iter_widgets (fun o -> o#unload_texture) room
...
let unload_textures room =
  let f r =
    unload_background r;
    match r.content with
    | Resident (Any w) -> w#unload_texture
    | _ -> () in
  iter f room
...
let set_keyboard_focus r =
  do_option r.keyboard_focus (fun b ->
      if not b then begin
        ...
        match r.content with
        | Rooms _ -> ()
        | Resident (Any w) -> Widget.set_keyboard_focus w
      end
    )
...
let rec remove_keyboard_focus r =
  ...
  match r.content with
    ...
    | Resident (Any w) -> Widget.remove_keyboard_focus w
    ...
    if not r.show then ()
        ...
        | _ -> begin
            ...
            then let label = Label.create ~size:7 ~fg:(Draw.(transp blue))
                 ...
                 List.iter
                   Draw.blit_to_layer
                   (Label.display (get_canvas r) (get_layer r) label geom)
          end
      end in
