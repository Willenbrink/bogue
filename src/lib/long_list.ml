...
(*        number of entries. *\) *)
(*   generate : (int -> 'a Widget.t); (\* the function to generate the ith entry *\) *)
(*   cleanup : ('a Widget.t -> unit); (\* cleanup the memory associated with the entry layout *\) *)
...
(*         else begin *)
(*           let mem = let (w,h) = l#size |> Draw.scale_size in w*h in *)
...
(*       let (w,h) = entry#size |> Draw.scale_size in *)
...
(*   let rec loop i ~h list : 'a Widget.t list * int = *)
...
(*       let dh = snd line#size in *)
(*       loop (i+1) ~h:(h+dh) (line::list) *)
(*     end *)
(*   in *)
(*   let list, i_final = loop i_start ~h:0 [] in *)
(*   let room = new Col.t ~name:"long_list room" ~sep:0 list in *)
...
(*         | None -> snd (get ll i direction)#size *)
(*         | Some dh -> dh *)
(*       in *)
